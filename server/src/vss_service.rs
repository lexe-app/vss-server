...
async fn handle_get_object_request(
	...
	trace!("Handling GetObjectRequest for key {}", request.key);
	let result = store.get(user_token, request).await;
	if let Err(ref e) = result {
		debug!("GetObjectRequest failed: {e}");
	}
	result
}
async fn handle_put_object_request(
	...
	if let Err(ref e) = result {
		debug!("PutObjectRequest failed: {e}");
	}
	result
}
async fn handle_delete_object_request(
	...
	trace!("Handling DeleteObjectRequest for key {:?}", request.key_value.as_ref().map(|t| &t.key));
	let result = store.delete(user_token, request).await;
	if let Err(ref e) = result {
		trace!("DeleteObjectRequest failed: {e}");
	}
	result
}
async fn handle_list_object_request(
	...
	if let Err(ref e) = result {
		debug!("ListKeyVersionsRequest failed: {e}");
	}
	result
}
